...
import {
    ...
    Truck, CheckCircle, Clock, XCircle, ChevronDown
} from 'lucide-react'
...
export default function AdminOrderDetailPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => api.get(`/orders`).then((r) => {
            const order = r.data.orders?.find((o) => o.id === id)
            return { order }
        }),
    })

    // Fetch single order via admin endpoint
    const { data: orderData } = useQuery({
        queryKey: ['admin-order-detail', id],
        queryFn: async () => {
            const res = await api.get('/orders', { params: { limit: 100 } })
            const order = res.data.orders?.find((o) => o.id === id)
            return { order }
        },
    })
    ...
    return (
        <div className="p-8">
            ...
            <div className="card p-4 mb-6 flex flex-wrap items-center gap-6">
                ...
                <div>
                    <p className="text-white/40 text-xs mb-0.5">Order Total</p>
                ...
                <div className="ml-auto">
                    <span className={`badge ${statusColors[order.status]} px-4 py-2`}>
                        {order.status}
                    </span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            {order.items?.map((item) => (
                                <div key={item.id} className="flex gap-4 p-4 bg-dark-600 rounded-xl">
                                    <div className="w-16 h-16 bg-dark-500 rounded-lg overflow-hidden flex-shrink-0">
                                        <img
                                            src={item.imageUrl || 'https://placehold.co/64x64/1a1a1a/333'}
                            ...
                            {order.statusHistory?.map((h, i) => (
                                <div key={h.id} className="flex gap-4">
                                    ...
                                    <div className="pb-6">
                                        ...
                                        <p className="text-white/25 text-xs mt-0.5">{new Date(h.createdAt).toLocaleString()}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="card p-6">
                        ...
                        <div className="space-y-2 text-sm">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="text-white">{Number(order.shippingFee) === 0 ? 'Free' : `$${Number(order.shippingFee).toFixed(2)}`}</span>
                            </div>
                            <div className="flex justify-between">
                            ...
                            {order.couponCode && (
                                <div className="flex justify-between">
                                    ...
                                    <span className="text-primary-400">{order.couponCode}</span>
                                </div>
                            )}
                    ...
                    <div className="card p-6">
                        ...
                        {order.address ? (
                            <div className="text-sm space-y-1">
                                ...
                                <p className="text-white/60">{order.address.country}</p>
                                <p className="text-white/40 mt-2">{order.address.phone}</p>
                            </div>
                        ) : (
                        ...
                        {order.payment ? (
                            <div className="space-y-2 text-sm">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span className={order.payment.status === 'PAID' ? 'text-green-400' : 'text-amber-400'}>
                                        {order.payment.status}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-white/50">Amount</span>
                                    <span className="text-white">${Number(order.payment.amount).toFixed(2)}</span>
                                </div>
                                {order.payment.paidAt && (
                                    <div className="flex justify-between">
                                        <span className="text-white/50">Paid At</span>
                                        <span className="text-white/70 text-xs">{new Date(order.payment.paidAt).toLocaleString()}</span>
                                    </div>
                                )}
                            </div>
                        ) : (
                            <p className="text-white/30 text-sm">No payment record</p>
                        )}
                    </div>
                    ...
                    {order.notes && (
                        <div className="card p-6">
                            <h2 className="font-display text-lg font-semibold text-white mb-3">Customer Notes</h2>
