...
export default function HeroSection() {
    return (
        <section className="relative min-h-[90vh] flex items-center overflow-hidden">
            ...
            <div className="container-custom relative z-10">
                <div className="max-w-3xl">
                    ...
                    <p className="text-white/50 text-lg md:text-xl mb-10 leading-relaxed max-w-xl animate-fade-up" style={{ animationDelay: '0.1s' }}>
                        Discover thousands of products across every category. Fast shipping, easy returns, and unbeatable prices.
                    </p>

                    {/* CTAs */}
                    <div className="flex flex-wrap gap-4 mb-16 animate-fade-up" style={{ animationDelay: '0.2s' }}>
                        <Link href="/products" className="btn-primary inline-flex items-center gap-2 text-base px-8 py-4">
                            Shop Now <ArrowRight size={18} />
                        </Link>
                        <Link href="/products?featured=true" className="btn-secondary inline-flex items-center gap-2 text-base px-8 py-4">
                            View Featured
                        </Link>
                    </div>
            ...
            <div className="absolute right-0 top-1/2 -translate-y-1/2 hidden xl:flex flex-col gap-4 pr-12">
                {[
                    { label: 'Orders Today', value: '1,240+' },
                    { label: 'Happy Customers', value: '50K+' },
                    { label: 'Products', value: '10K+' },
                ].map((stat) => (
                    <div key={stat.label} className="card px-6 py-4 text-right animate-fade-in">
